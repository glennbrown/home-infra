...
# git submodule - update to latest remote commit (optional submodule name, updates all if omitted)
update-submodule *NAME:
    #!/usr/bin/env sh
    if [ -z "{{ NAME }}" ]; then
        git submodule update --remote --merge
        git add .
        git commit -m "Updates all submodules to latest"
    else
        git submodule update --remote --merge "roles/{{ NAME }}"
        git add "roles/{{ NAME }}"
        git commit -m "Updates {{ NAME }} submodule to latest"
    fi
